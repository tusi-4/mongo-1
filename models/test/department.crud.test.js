...
describe('Department', () => {
  ...
  describe('Updating data', () => {
    ...
    it('should properly update multiple documents with "updateMany" method', async () => {
      // metoda własna
      await Department.updateMany([{ name: 'Department #1' }, { $set: { name: '=Department #1=' }}, { name: 'Department #2' }, { $set: { name: '=Department #2='} }]);
      const updatedDepartments = await Department.find({ name: '=Department #1=' }, { name: '=Department #2='});
      expect(updatedDepartments).to.not.be.null;
    });
  ...
  describe('Removing data', () => {
    ...
    it('should properly remove one document with "deleteOne" method', async () => {
      ...
      expect(removedDepartment).to.not.exist;
    });
